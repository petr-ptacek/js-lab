...
describe("scaleByAspectRatio", () => {
  it("scales by width when target.width is provided", () => {
    const result = scaleByAspectRatio({ width: 400, height: 200 }, { width: 200 });
  ...
  it("scales by height when target.height is provided", () => {
    const result = scaleByAspectRatio({ width: 400, height: 200 }, { height: 100 });
  ...
  it("uses custom rounding function", () => {
    ...
    const result = scaleByAspectRatio({ width: 1000, height: 333 }, { width: 500 }, floor);
  ...
  it("throws for invalid size values", () => {
    expect(() => scaleByAspectRatio({ width: 0, height: 300 }, { width: 100 })).toThrow();

    expect(() => scaleByAspectRatio({ width: 400, height: -1 }, { height: 100 })).toThrow();
  });

  it("throws for invalid target values", () => {
    expect(() => scaleByAspectRatio({ width: 400, height: 300 }, { width: 0 })).toThrow();

    expect(() => scaleByAspectRatio({ width: 400, height: 300 }, { height: -50 })).toThrow();
  });
});
