...
describe("clampPercentWithLimits", () => {
  describe("base percent clamp", () => {
    it.each([
      ...
      [50, 200, 50],
      [150, 200, 100],
    ])("clamps percent %o to %o", (percent, containerSize, expected) => {
      expect(clampPercentWithLimits(percent, containerSize, {})).toBe(expected);
    });
  });
  ...
  describe("minSize", () => {
    it("respects minSize in px", () => {
      expect(clampPercentWithLimits(10, 200, { minSize: 50 })).toBe(25);
    });

    it("respects minSize in percent", () => {
      expect(clampPercentWithLimits(10, 200, { minSize: "30%" })).toBe(30);
    });

    it("does not exceed 100%", () => {
      expect(clampPercentWithLimits(90, 200, { minSize: 300 })).toBe(100);
    });
  });

  describe("maxSize", () => {
    it("respects maxSize in px", () => {
      expect(clampPercentWithLimits(80, 200, { maxSize: 100 })).toBe(50);
    });

    it("respects maxSize in percent", () => {
      expect(clampPercentWithLimits(80, 200, { maxSize: "60%" })).toBe(60);
    });

    it("does not go below 0%", () => {
      expect(clampPercentWithLimits(10, 200, { maxSize: -50 })).toBe(0);
    });
  });
