...
describe("scaleToWidth", () => {
  it("scales height based on width and aspect ratio", () => {
    const result = scaleToWidth({ width: 400, height: 200 }, 200);
  ...
  it("uses custom rounding function", () => {
    ...
    const result = scaleToWidth({ width: 1000, height: 333 }, 500, floor);
  ...
  it("throws for invalid input", () => {
    expect(() => scaleToWidth({ width: 0, height: 100 }, 50)).toThrow();

    expect(() => scaleToWidth({ width: 100, height: 100 }, 0)).toThrow();
  });
});
